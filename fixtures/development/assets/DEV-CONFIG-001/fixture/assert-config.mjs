// DEV-CONFIG-001 fixture: assert the effective npm boolean for `fund`.
//
...
// for the effective value and compares it against the frozen expected project
// value `fund=true`. In A arms the environment source `npm_config_fund=false`
// wins under real npm precedence and the assertion fails. No registry contact
// or file mutation happens here.
...
const effective = npmConfigGet("fund");
if (effective.error !== undefined || effective.status !== 0) {
  ...
  process.stdout.write("RUNPARITY_OK:dev-config-001\n");
  process.exitCode = 0;
} else if (effective.stdout.trim() === "false") {
  process.stderr.write("RP_FIXTURE_ENV_OVERRIDES_PROJECT_NPMRC\n");
  process.exitCode = 23;
} else {
  process.stderr.write("RP_FIXTURE_INVALID_CONFIG_ASSERTION\n");
  process.exitCode = 64;
