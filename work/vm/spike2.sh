...
echo "probe exit=$?"
head -c 900 vm-spike-bundle.json; echo
...
$SSH "podman run --rm --name rp-spike-det --network none --cap-drop ALL $IMG sh -c 'sleep 300 & echo spawned; exit 0'" 2>&1 | tail -1
sleep 2
LEFT=$($SSH "podman ps -a --filter name=rp-spike-det --format json" 2>/dev/null)
echo "leftover=$LEFT"
CNT=$($SSH "pgrep -c -u 1000 -x sleep" 2>/dev/null || true)
echo "surviving_sleep_count_exit=$?"

echo "=== spike: residue probes ==="
$SSH 'rm -rf /home/rp/arms/spike1 && mkdir -p /home/rp/arms/spike1'
$SSH "podman run --rm --network none --cap-drop ALL --security-opt no-new-privileges --read-only --userns keep-id:uid=10001,gid=10001 --user 10001:10001 -v /home/rp/probe:/probe:ro -v /home/rp/arms/spike1:/home/arm:rw $IMG node /probe/residue-probe.mjs --write"
$SSH 'rm -rf /home/rp/arms/spike2 && mkdir -p /home/rp/arms/spike2'
$SSH "podman run --rm --network none --cap-drop ALL --security-opt no-new-privileges --read-only --userns keep-id:uid=10001,gid=10001 --user 10001:10001 -v /home/rp/probe:/probe:ro -v /home/rp/arms/spike2:/home/arm:rw $IMG node /probe/residue-probe.mjs --check"
